...
try {
  await step('default map, eight cards, and listing order', async () => {
    await open();
    assert.equal(await page.locator('#car-discovery').isVisible(), true);
    assert.equal(await cards(), await onPage());
    assert.ok(await page.locator('.car-price-marker').count() > 0);
  ...
  await step('missing coordinates keep every listing accessible', async () => {
    ...
    assert.equal(await page.locator('.car-price-marker').count(), 0);
